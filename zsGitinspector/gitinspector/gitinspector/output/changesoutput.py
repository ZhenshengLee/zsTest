...
class ChangesOutput(Outputable):
    ...
    def output_xlsx(self):
        print("Please go to see the excel file in current path!")

        workbook = xlsxwriter.Workbook('git-output.xlsx')
        ...
        if authorinfo_list:
            worksheet.write('A1', 'Author', cell_format_bold)
            worksheet.write('B1', 'Commits', cell_format_bold)
            worksheet.write('C1', 'Insertions', cell_format_bold)
            worksheet.write('D1', 'Deletions', cell_format_bold)
            worksheet.write('E1', '% of changes', cell_format_bold)
            ...
            for i in sorted(authorinfo_list):
                ...
                worksheet.write(row, col, terminal.ljust(i, 20)[0:20 - terminal.get_excess_column_count(i)])
                col+=1
                worksheet.write_number(row, col, (authorinfo.commits))
                col+=1
                worksheet.write_number(row, col, (authorinfo.insertions))
                col+=1
                worksheet.write_number(row, col, (authorinfo.deletions))
                col+=1
                worksheet.write_number(row, col, (percentage), cell_format_percent)
                row+=1

            chart_pie = workbook.add_chart({'type': 'pie'})
            chart_pie.add_series({
                'name': 'data',
                'categories': ('=Sheet1!$A$2:$A$'+str(row)),
                'values': ('=Sheet1!$E$2:$E$'+str(row)),
            })
            chart_pie.set_title({'name': 'modification of authors'})
            worksheet.insert_chart('A11', chart_pie, {'x_offset': 25, 'y_offset': 10})
        else:
            print(_(NO_COMMITED_FILES_TEXT) + ".")
